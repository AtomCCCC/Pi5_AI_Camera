...
| **Servo feedback control** | Dual-axis visual PID with deadband, anti-windup, filtering, and safety limits |
...
python -m control_service.main           # Visual PID -> pan/tilt commands
...
| `vision/detections` | Vision Service | Control Service | `{detections, frame_size, coordinate_space}` |
...
| `gpio/command` | LLM Orchestrator / Control Service | GPIO Service | `{type, servo, angle, session_id}` |
| `control/status` | Control Service | Dashboard / observers | `{mode, pan, tilt, pid_active, latency_ms}` |
