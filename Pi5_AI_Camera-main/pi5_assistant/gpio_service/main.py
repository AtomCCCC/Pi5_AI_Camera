...
class GPIOService:
    ...
    def _on_command(self, payload):
        ...
        if cmd_type == "servo":
            servo = payload["servo"]
            angle = payload["angle"]
            # PWM writes are short. Keeping them on this serial callback path
            # prevents continuous PID commands from completing out of order.
            try:
                self.servo.set_angle(servo, angle)
            except (TypeError, ValueError, OSError) as exc:
                logger.warning("Rejected servo command: %s", exc)
